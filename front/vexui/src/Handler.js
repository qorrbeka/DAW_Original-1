...
Vex.UI.Handler.prototype.createStaves = function() {
	...
	for(var i = 0; i < this.options.numberOfStaves; i++){
		...
		if (i === 0) {
			stave.setBegBarType(Vex.Flow.Barline.type.SINGLE); // ù ��° �Ǻ��� ���� �ݺ��� �߰�
		}
		else if (i === this.options.numberOfStaves - 1) {
			stave.setEndBarType(Vex.Flow.Barline.type.END); // ������ �Ǻ��� �� �ݺ��� �߰�
		}
		else {
			stave.setEndBarType(Vex.Flow.Barline.type.SINGLE); // ������ �Ǻ��� �⺻ ���� �� �߰�
		}
...
Vex.UI.Handler.prototype.drawNotes = function(stave){
	if(stave === undefined){
		...
		//var voice = new Vex.Flow.Voice(Vex.Flow.TIME3_4).setMode(Vex.Flow.Voice.Mode.SOFT);
	...
	if(this.provisoryTickable && this.currentStave!=null){
		...
		if(mousePos!==undefined){
			if(this.provisoryTickable instanceof Vex.Flow.StaveNote){
				...
				//this.provisoryTickable.x_shift= mousePos.x - this.provisoryTickable.getAbsoluteX();
...
Vex.UI.Handler.prototype.play = function(callback){
	...
	for(var i = 0; i < this.staveList.length; i++){
        ...
        //eel.debug_log(stave.getTickables());
        // console.log(stave.getTickables());
...
Vex.UI.Handler.prototype.save = function() {
    eel.debug_log("save start");
    eel.save_to_midi_file();
};

Vex.UI.Handler.prototype.load = async function() {
    eel.debug_log("load start");
    let note_array = await eel.load_from_midi_file()();
    console.log('Array = ' + note_array);

    var key;
    var j = 0;
    var stave;
    var noteLists_from_load;
    for(var i = 0; i < this.staveList.length; i++){
		stave = this.staveList[i];
        noteLists_from_load = [];

        for(; j < note_array.length; j++) {
            switch(note_array[j] % 12) {
                case 0 :
                    key = "C/";
                    break;
                case 2 :
            ...
            key = key + Math.floor(note_array[j]/12)

            var note = new Vex.Flow.StaveNote({keys: [key], duration: "4"});
		    noteLists_from_load.push(note);
		    console.log(note);
        ...
        stave.setTickables(noteLists_from_load);
	}
