...
Vex.UI.MouseListener.prototype.handleLeftMouseClick = function(evt){
	if(this.handler.currentStave!=null){
		...
		if(this.handler.currentNote==null){
            ...
            var noteValue = newNote.keyProps[0].int_value;
            var durationValue = newNote.duration;
			// eel.debug_log(typeof noteValue);
			eel.save_note_on(noteValue, durationValue);
			// console.log(newNote.keyProps[0].int_value);

			//Redraw the stave
			this.handler.redraw();
			
		} else{
		    eel.debug_log("clicked 2");
